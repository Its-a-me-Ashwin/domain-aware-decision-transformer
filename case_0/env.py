...
class DoubleInvertedPendulumBalanceEnv(gym.Env):
    ...
    def __init__(self,
        ...
        high = np.array([np.finfo(np.float32).max]*9, dtype=np.float32)
        self.observation_space = spaces.Box(-high, high, shape=(9,), dtype=np.float32)
    ...
    def reset(self, seed=0):
        ...
        p.resetJointState(self.box_id, 2, offset2, 0.0, physicsClientId=self.client_id)
    ...
    def _get_observation(self):
        ...
        obs = np.array([
            ...
            positions[1],           # p1_theta
            velocities[1],          # p1_theta_dot
            accelerations[1],       # p1_theta_acc
            positions[2],           # p2_theta
            velocities[2],          # p2_theta_dot
            accelerations[2],       # p2_theta_acc
        ], dtype=np.float32)
